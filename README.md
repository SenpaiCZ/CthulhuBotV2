...
The bot supports **Discord Activities** (Embedded App SDK), allowing users to launch an interactive Character Sheet directly inside Discord.

### 1. Developer Portal Configuration
1.  Go to the [Discord Developer Portal](https://discord.com/developers/applications).
2.  Select your application.
3.  Copy the **Client ID** and **Client Secret** (you will need these for `config.json`).
4.  Go to the **OAuth2** tab.
5.  Add a **Redirect URI**. This must be the public URL of your dashboard + `/activity/callback`.
    *   Example: `https://your-bot-domain.com/activity/callback`
    *   *If testing locally with ngrok:* `https://<random-id>.ngrok-free.app/activity/callback`

### 2. Bot Configuration
Update your `config.json` (or use environment variables) with the following:
...
```json
{
    ...
    "discord_redirect_uri": "https://your-bot-domain.com/activity/callback"
}
```

### 3. Hosting (Public Access Required)
Discord Activities require a valid **HTTPS** URL that is publicly accessible. If you are hosting locally (e.g., on a Raspberry Pi), you need to expose the dashboard port (default 5000).

#### Option A: Cloudflare Tunnel (Recommended)
1.  Install `cloudflared`.
2.  Run a tunnel pointing to your local dashboard port:
    ```bash
    cloudflared tunnel --url http://localhost:5000
    ```
3.  Copy the generated URL (e.g., `https://random-name.trycloudflare.com`) and use it as your base URL for the Redirect URI.
...
### 4. Configuring the Activity in Discord
To enable the "Activity" button in Discord for your bot, you currently need access to the Embedded App SDK developer preview or wait for public release. However, you can test it by manually constructing the URL or using a "Launch Activity" command if you implement one (currently, users just click the link generated by the OAuth flow if you provide it, or use a proxy app).

*Note: As of now, Discord Activities are often launched via specific entry points. This bot implements the web view portion. You may need to create an interaction that replies with a link to `https://your-domain.com/activity` for users to click.*
