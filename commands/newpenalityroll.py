...
class newpenalityroll(commands.Cog):
    def __init__(self, bot):
        self.bot = bot

    @commands.command(aliases=["NPD", "npd", "ps"], guild_only=True)
    async def newpenalityroll(self, ctx, *, dice_expression):
        ...
        if user_id not in player_stats[server_id]:
            ...
            try:
                ...
                embed = discord.Embed(
                    ...
                    description=f"{ctx.author.mention} :game_die: Rolled: {tens_rolls[0]}|{tens_rolls[1]} + {ones_roll} = {roll}\n{result}\n{formatted_skill}\n{formatted_luck}",
                    color=discord.Color.green(),
                )

                luck_stats = await load_luck_stats()
                LUCKTHR = luck_stats[server_id] if server_id in luck_stats else 10
                if (
                    roll > current_value
                    and roll <= current_value + LUCKTHR
                    and player_stats[server_id][user_id]['LUCK'] >= roll - current_value
                    and stat_name != "LUCK"
                ):
                    ...
                    try:
                        ...
                        if str(reaction.emoji) == "✅":
                            luck_used = min(
                                player_stats[server_id][user_id]['LUCK'], difference
                            )
                            player_stats[server_id][user_id]['LUCK'] -= luck_used
                            formatted_luck = f":four_leaf_clover: LUCK: {player_stats[server_id][user_id]['LUCK']}"
                        ...
                        embed = discord.Embed(
                            title=f"{ctx.author.display_name}'s Skill Check for '{stat_name}'",
                            description=f"{ctx.author.mention} :game_die: Rolled: {tens_rolls[0]}|{tens_rolls[1]} + {ones_roll} = {roll}\n{result}\n{formatted_skill}\n{formatted_luck}",
                            color=discord.Color.green(),
                        )
                    except asyncio.TimeoutError:
                ...
                if SUCCESSFULLROLL == 1 and ASKFORSESSION == 1:
                    ...
                    await session_message.add_reaction("✅")  # Add checkmark emoji
...
async def setup(bot):
    await bot.add_cog(newpenalityroll(bot))
